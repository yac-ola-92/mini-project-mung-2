...
public class AccomController {
    ...
    // 중복을 제거 헬퍼 함수 filter -> distinctByKey
    ...
    public String go(Model model, HttpSession session){
        ...
        List<AccomDTO> distinctList = list.stream()
                .filter(distinctByKey(AccomDTO::getAccom_id))
        ...
        if(check ==null){
           model.addAttribute("accomRating", distinctList);
        }else {
    ...
    @GetMapping("/registerForm") //숙소 등록 페이지 이동
